...
class NumberoAPIClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        Args:
            api_key: Numbeo API key. If not provided, will try to read from NUMBEO_API_KEY env var.
        """
        self.api_key = api_key or os.getenv("NUMBEO_API_KEY")
        if not self.api_key:
            raise ValueError(
                "Numbeo API key is required. Set NUMBEO_API_KEY environment variable or pass api_key parameter."
            )
